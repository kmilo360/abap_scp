...
CLASS zcl_renting_load_data1 IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.
    DATA: LT_BRANDS TYPE TABLE OF zrent_brands1,
          LT_CARS TYPE TABLE OF zrent_cars1,
          LT_CUSTOMERS TYPE TABLE OF zrent_customers1.
*   * BRANDS **
    lt_brands = VALUE #(
    ...
    DELETE FROM zrent_brands1.
    INSERT zrent_brands1 FROM TABLE @lt_brands.

    SELECT * FROM zrent_brands1 INTO TABLE @lt_brands.

    out->write( sy-dbcnt ).
    out->write( 'zrent_brands1 data inserted successfully!').
...
*   * CARS**
    ...
    DELETE FROM zrent_cars1.
    INSERT zrent_cars1 FROM TABLE @lt_cars.

    SELECT * FROM zrent_cars1 INTO TABLE @lt_cars.
    out->write( sy-dbcnt ).
    out->write( 'zrent_cars1 data inserted successfully!').
*    * CUSTOMERS**
    lt_customers = VALUE #(
        ...
        DELETE FROM zrent_customers1.
        INSERT zrent_customers1 FROM TABLE @lt_customers.

        SELECT * FROM zrent_customers1 INTO TABLE @lt_customers.
        out->write( sy-dbcnt ).
        out->write( 'zrent_customers1 data inserted successfully!').
